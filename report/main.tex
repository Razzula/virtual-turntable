...
\begin{filecontents*}{\jobname.xmpdata}
    ...
    \section{Design} % DESIGN / METHODOLOGY
        ...
        \subsection{System Architecture} % high-level system overview
            ...
            \subsubsection{System Stack}
                ...
                The system stack includes FastAPI, React, and Raspberry Pi 5; specific tools were chosen based on suitability for task and are detailed in the sections below.
        ...
        \subsection{Machine Learning Model Design} \label{sec:mlDesign}
            ...
            \subsubsection{Model Architecture}
                ...
                The simplest option is to re-train the whole model, with the original and additional classes and data. However, in order to best maintain legal fair dealings, all images used in training were deleted after use. Re-downloading the dataset each time would be impractical, especially as some images may later be removed from the source, potentially degrading model performance if retrained.
        ...
        \subsection{Security Considerations} \label{sec:security}
            ...
            \paragraph{Local Network Access} To support remote control, Vite's network-hosting allows LAN devices to connect. As the system is intended for home use, this is considered safe, provided firewall protections are in place.
    ...
    \section{Implementation}
      ...
      \subsection{Machine Learning Model} \label{sec:mlImp}
            ...
            % \begin{table}[h]
            %     \centering
            %     \caption{Comparison: Raspberry Pi 5 vs RTX 3060 Ti for CNN Training}
            %     \label{tab:trainingComparison}
            %     \begin{tabular}{|l|c|c|}
            %     \hline
            %     \textbf{Metric} & \textbf{Raspberry Pi 5} & \textbf{NVIDIA RTX 3060 Ti} \\
            %     \hline
            %     Architecture & ARM Cortex-A76 CPU + VideoCore VII GPU & Ampere GPU, 4864 CUDA cores \\
            %     \hline
            %     General-Purpose GPU Compute & Not supported & Fully supported (CUDA) \\
            %     \hline
            %     RAM & 8 GB LPDDR4 & Typically 16–32 GB system RAM + 8 GB GDDR6 \\
            %     \hline
            %     Training Time (small CNN, ~1k images) & ~4–6 hours & <1 minute \\
            %     \hline
            %     Feasibility for moderate dataset (e.g., >10k images) & Impractical & Fully suitable \\
            %     \hline
            %     Support for Tensor Acceleration & Limited (CPU only, no CUDA) & Full (Tensor cores, CUDA, cuDNN) \\
            %     \hline
            %     Usability for Model Inference & Possible (small models only) & Excellent \\
            %     \hline
            %     Thermal/Power Constraints & Tight; may throttle under load & None (desktop-class cooling) \\
            %     \hline
            %     \end{tabular}
            % \end{table}
